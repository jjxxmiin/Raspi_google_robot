...
class SampleAssistant(object):
    ...
    def assist(self):
        ...
        '''
        메인에서 사용하기 위해서 리턴값 추가
        '''
        return continue_conversation, stt_list
...
def main (api_endpoint, credentials, project_id,
    ...
    with SampleAssistant(lang, device_model_id, device_id,
        ...
        while True:
            ...
            continue_conversation, stt_list = assistant.assist()
            # wait for user trigger if there is no follow-up turn in
            # the conversation.
            wait_for_user_trigger = not continue_conversation
            ...
            if once and (not continue_conversation):
                break
            '''
            Speech가 들어오면 조금조금씩 문장을 완성시켜서 맨 마지막에 완성된 말을
            인식하기 때문에 마지막만 출력해준다.
            여기서 부터 가져온 값을 가지고 동작을 진행하면 된다.
            '''
...
현재 Speech를 Text로 변환된 값을 가져오는 것은 가능하고 motor를 동작시키는 부분만 짜면 될것 같다.
